...
class Best_experience_Buffer(EpisodeBatch):
    def __init__(self, scheme, groups, buffer_size, max_seq_length, preprocess=None, device="cpu"):
        super(Best_experience_Buffer, self).__init__(
            scheme, groups, buffer_size, max_seq_length, preprocess=preprocess, device=device)
    ...
    def insert_episode(self, ep):
        if self.episodes_in_buffer < self.buffer_size:
            tot_r = torch.sum(ep['reward'], dim=1).item()
