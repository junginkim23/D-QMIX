...
def get_config_mpe(args):
    ...
    with open(os.path.join(dirname(os.path.dirname(__file__)), "config", 'envs', "{}.yaml".format('mpe_env')), "r") as f:
        try:
            config_dict = yaml.load(f, Loader=yaml.SafeLoader)
    ...
    final_config_dict['scenario_name'] = args.map_name
    final_config_dict['map_name'] = args.map_name
    final_config_dict['t_max'] = args.t_max
    final_config_dict['epsilon_anneal_time'] = args.anneal_time
    final_config_dict['seed'] = args.seed
    final_config_dict['num_cur_step'] = args.num_cur_step
