...
def onehot_from_logits(logits, eps=0.0, dim=1):
    ...
    rand_acs = th.eye(logits.shape[1]).requires_grad_(False)[[np.random.choice(range(logits.shape[1]), size=logits.shape[0])]]
...
def sample_gumbel(shape, eps=1e-20, tens_type=th.FloatTensor):
    ...
    U = tens_type(*shape).uniform_().requires_grad_(False)
...
def run(_run, _config, _log):
    ...
    if args.use_tensorboard:
        # tb_logs_direc = os.path.join("./results", "{}/tb_logs_{}/anneal_time_{}".format(game_name, args.seed, args.epsilon_anneal_time))
        tb_exp_direc = os.path.join(args.local_results_path, f"{unique_token}")
...
def run_sequential(args, logger):
    ...
    runner.setup(scheme=scheme, groups=groups, preprocess=preprocess, mac=mac, opponents=opponents) # ADD NEW (opponents)
